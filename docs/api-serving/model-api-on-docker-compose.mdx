...
2. Docker Compose 파일을 작성합니다.
    - Model API 서버
        - <var>Service name</var>: `api-with-model`
        - <var>Image</var>: `Dockerfile`
        - <var>Container name</var>: `api-with-model`
        - <var>Port</var>: `8000:8000`
...
- <var>healthcheck</var> :
  ...
  - curl 을 이용하여 API 서버의 동작을 테스트하는 코드를 추가합니다. 정상 동작이 확인되면 서비스를 띄웁니다.
