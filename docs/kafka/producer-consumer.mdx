...
services:
    ...
    environment:
      ...
      ZOOKEEPER_CLIENT_PORT: 2181
```
- `image` : zookeeper 를 띄울 때 쓰이는 image 를 명시합니다. 이번 챕터에서는 `confluentinc/cp-zookeeper:7.3.0` 를 이용하도록 하겠습니다.
- `container_name` : container 의 이름을 지정합니다. 이번 챕터에서는 `zookeeper` 로 사용하겠습니다.
- `ports` : zookeeper 의 port 를 지정합니다. 이번 챕터에서는 `2181:2181` 을 사용하겠습니다.
