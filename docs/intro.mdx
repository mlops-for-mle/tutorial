...
이를 위해서는 OS, 파이썬 버전, 패키지 버전, 코드, 가중치 등 모든 것이 모델을 학습했을 때의 환경과 동일해야 합니다.
이런 동일한 환경을 제공하기 위해서  Containerization 은 머신러닝에서 중요한 기술 중 하나이며 이를 대표하는 소프트웨어가 바로 Docker 입니다.
이러한 배경으로 머신러닝 엔지니어는 Docker를 이해하고 다룰 수 있어야 합니다. 머신러닝 엔지니어가 Docker를 다루고 이해할 수 있도록 본 문서에서는 로컬 환경에서 실행되는 스크립트를 작성한 후 이를 Docker를 사용해 실행하는 순서로 진행됩니다.
...
예를 들어 다음에 학습할 챕터sms 아래와 같이 되어 있습니다.
