...
**< 아래의 그림에서 모델 배포 뒤에 모델 서빙이 붙어야할 것 같습니다. >**
...
**< 아래의 그림에서 table creator 이미지가 빠진 것 같습니다. >**
...
이러한 배경으로 머신러닝 엔지니어에게 Docker를 이해하고 다룰 수 있어야 합니다. 머신러닝 엔지니어에게 필요한 수준의 docker 를 다루고 이해할 수 있게하기 위해서 본 문서에서는 로컬 환경에서 실행되는 스크립트 작성 후 이를 docker 에서도 실행할 수 있는 순서로 진행됩니다.

**< 대표하는 용어들을 글 중간에서 표현할 때 대문자로 할 것인지, 소문자로 할 것인지 규칙이 필요합니다. ex) Docker 로 표현할 것인지, docker 로 표현할 것인지? >**
...
2. Postgres 서버를 생성합니다.
    ...
    - Port Forwarding : `5432:5432` **< 앞에서는 port forwarding 으로 쓰고, 뒤에서는 port 로 쓰여진 것 같습니다. 통일성이 필요해보입니다. >**
3. 생성된 postgres 서버를 확인합니다.
    - `psql` 로 DB 에 접근하여 role name 과 attributes 확인

**< ` ` 하고 bold 글씨의 용도를 정해야할 것 같습니다. ` ` 은 언제 써도 되며, bold 글씨는 언제써도 되는걸까요? >**
