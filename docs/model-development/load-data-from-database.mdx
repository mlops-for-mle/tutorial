...
1. 데이터 불러오기
    -  <Part>01. Database</Part> 파트에서 생성한 데이터베이스에서 데이터를 가져옵니다.
...
해당 파트의 전체 코드는 [mlops-for-mle/ch2/](https://github.com/mlops-for-mle/mlops-for-mle/tree/main/ch2) 에서 확인할 수 있습니다.
...
📌  해당 챕터는 <Part>01. Database</Part> 파트의 DB를 이용합니다.  
...
이번 챕터에서 사용할 패키지들인 `psycopg2-binary` 를 설치합니다.
...
DB에 연결하기 위한 정보는 <Part>01. Database</Part> 의 <Chapter>6) Data Generator on Docker Compose</Chapter> 챕터에서 postgres 서버를 생성할 때 입력한 값입니다.
...
<Chapter>2) Model Pipeline</Chapter> 에서 작성한 `base_train.py` 의 `# 1. get data` 부분을 위에서 작성한 코드로 수정합니다.
...
## 2. Save Data
그리고 사용한 데이터를 저장하는 `# 2. save data` 부분을 추가합니다.
...
그리고 `# 1. reproduce data` 에서 저장된 데이터를 불러오게 수정합니다.
