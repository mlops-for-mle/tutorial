...
1. 데이터 불러오기
    ...
    - <code>id</code> column을 기준으로 최신 데이터 100개를 추출하는 쿼리문을 작성합니다.
    - <code>pandas.read_sql</code> 함수를 이용해 데이터를 추출합니다.
2. 모델 파이프라인 수정
...
📌  DB 를 띄우지 않은 경우 <Part>01. Database</Part> 을 완료하고 DB 가 띄워진 상태에서 진행해주세요.
...
DB 에 연결하기 위한 정보는 <Part>01. Database</Part> 의 <Chapter>6) Data Generator on Docker Compose</Chapter> 챕터에서 DB 서버를 생성할 때 입력한 값입니다.
