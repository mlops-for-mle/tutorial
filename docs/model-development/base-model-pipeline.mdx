...
## 1. Model Pipeline
### 1.1 Scaler & SVC
이 전 장에서 예측을 위해 사용한 모델은 scaler 와 SVC 두 가지가 있었습니다.
또한 SVC가 정상적으로 예측하기 위해서는 scaler가 필요하다는 것도 알아 보았습니다.
...
하지만 위 방법은 scaler 를 사용하지 않거나 다른 scaler를 사용하는 경우가 발생할 수 있습니다.
이러한 실수를 막을 수 있는 방법이 모델들을 파이프라인화 시키는 것입니다.
파이프라인화 된 모델은 아래 처럼 사용할 수 있습니다.
...
마지막으로 모델을 저장합니다. <Chapter>1) Base Model Development </Chapter> 챕터처럼 따로 할 필요 없이 하나의 파일로 저장할 수 있습니다.
