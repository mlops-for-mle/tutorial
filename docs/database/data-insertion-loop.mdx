...
2. 스크립트를 실행하여 <var>`iris_data`</var> 테이블에 계속해서 데이터를 삽입합니다.
...
여기서는 추가적으로 <code>while</code> 문을 사용하여 코드를 작성하겠습니다. <code>while True</code> 를 통해 외부의 개입이 없다면 계속해서 while 문을 실행할 수 있습니다.
...
<Chapter>3) Data Insertion</Chapter> 챕터에서 작성한 내용들과 이번 챕터에서 설명한 내용을 모아서 `data_insertion_loop.py` 로 작성합니다.
