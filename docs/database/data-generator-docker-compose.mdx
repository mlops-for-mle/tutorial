...
- `command` : Dockerfile에 작성되어 있는 `CMD` 를 덮어씁니다.<Chapter>5) Data Generator on Docker</Chapter> 챕터에서 DB 의 host 는 컨테이너의 이름으로 주어야 한다는 것을 설명했습니다. 이를 위해서 여기서는 `["postgres-server"]` 로 덮어씁니다.
