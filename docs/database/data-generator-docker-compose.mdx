---
sidebar_position: 6
description: 📌 DB 컨테이너와 data generator 컨테이너를 함께 띄우기 위한 `docker-compose.yaml` 을 작성합니다.
---
...
1. DB 컨테이너와 data generator 컨테이너를 함께 띄우기 위한 `docker-compose.yaml` 파일을 작성합니다.
...
1. <code>docker-compose.yaml</code> 을 작성합니다.
...
이제 `docker-compose.yaml` 을 작성해보도록 하겠습니다.
...
- `version` : `Compose` 파일의 버전을 의미합니다. 최신 버전은 [공식 홈페이지](https://docs.docker.com/compose/compose-file/compose-versioning/)에서 확인하실 수 있습니다.
  이번 챕터에서는 작성일 기준 최신 버전인 `"3"` 을 사용하겠습니다.
- `services` : `Compose` 에 묶일 서비스들을 의미합니다. 하나의 서비스는 하나의 컨테이너를 의미합니다.
  - `postgres-server` : <Chapter>01) DB Server Creation</Chapter> 챕터에서 사용한 DB 서버를 실행시키기 위한 내용을 정의할 서비스 이름입니다.
  - `data-generator` : <Chapter>05) Data Generator on Docker</Chapter> 챕터에서 사용한 data generator 를 실행시키기 위한 내용을 정의할 서비스 이름입니다.
...
이제 `docker-compose.yaml` 에 정의되어 있는 서비스들의 컨테이너를 실행해보겠습니다.
...
Postgres server 는 아직 준비가 되어있지 않은데 data generator 가 띄워져서 DB connection 을 하려다보니 data generator 가 Exited 되는 문제가 발생한 것입니다.
