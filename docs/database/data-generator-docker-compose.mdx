...
DB service 와 data generator service 를 띄우는 `docker-compose.yaml` 파일을 작성해보도록 하겠습니다.

`docker-compose.yaml` 은 다음과 같이 형태로 구성되어 있습니다.
...
`docker exec` 를 이용하여 data generator container 안으로 접속하고, `psql` 을 이용하여 DB 로 접속합니다. 이 때 local 이 아닌 data generator container 에서 접속해야 하기 때문에 host 를 `localhost` 에서 `postgres-server` 로 변경합니다.
