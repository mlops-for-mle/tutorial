...
1. Docker Compose 파일을 작성합니다.
    ...
    Postgres server 서비스와 data generator 서비스를 띄울 때 어떤 서비스가 먼저 띄워져야 하는지 생각해보고, 그 기능을 Compose 파일에 추가합니다.
...
- `version` : Compose 파일의 버전을 의미합니다. 최신 버전은 [공식 홈페이지](https://docs.docker.com/compose/compose-file/compose-versioning/)에서 확인하실 수 있습니다.
  이번 챕터에서는 작성일 기준 최신 버전인 3을 사용하겠습니다.
...
`psql` 을 이용하여 DB 에 접속하여 계속해서 데이터가 삽입되고 있는지 확인합니다.
