...
1.  <Chapter>4) Data Insertion Loop </Chapter> 챕터에서 작성한 script 를 build 할 수 있는 `Dockerfile` 을 작성합니다.
...
### 1.1 `data_generator.py`
 ...
 <Chapter>4) Data Insertion Loop </Chapter> 챕터에서 작성한 코드들을 모아서 데이터를 생성하는 `data_generator.py` 를 작성합니다.
...
[그림 1-2] 는 컨테이너를 띄운 데스크탑과 컨테이너의 관계입니다.  <Chapter>1) DB Server Creation </Chapter> 챕터에서 DB 컨테이너를 띄울 때 사용한 명령어는 다음과 같습니다.
...
2.  <Chapter>3) Data Insertion </Chapter> 챕터에서 작성한 `iris_data` 테이블에 있는 데이터 전체를 확인하기 위한 query 를 실행합니다.
