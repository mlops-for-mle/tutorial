...
## 1. 테이블 생성
Python 언어를 이용하여 postgres 서버에 접근하는 코드를 구현하는 가장 간단한 방법은 `psycopg2` 패키지를 이용하는 것입니다.  
...
`create_table`  의 입력값은 <var>`db_connect`</var> 입니다. DB 와의 연결을 지속적으로 요구할 경우 부하가 갈 수 있기 때문에 최초에 한 번만 연결 후 연결된 connector 를 재사용하기 위함입니다.  
