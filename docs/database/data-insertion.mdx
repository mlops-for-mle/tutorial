...
2. `psycopg2` 패키지를 이용하여 생성된 <var>`iris_data`</var> 테이블에 데이터 row 1개를 삽입합니다.
...
Iris 데이터 를 불러온 뒤,  <Chapter>2) Table Creation</Chapter> 챕터에서 생성된 테이블의 column 이름과 일치하도록 수정합니다.
...
2. <Chapter>2) Table Creation</Chapter> 챕터에서 작성한 <var><code>iris_data</code></var> 테이블에 있는 데이터 전체를 확인하기 위한 query 를 실행합니다.
