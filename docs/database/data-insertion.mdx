...
Iris 데이터 를 불러온 뒤,  <Highlight color="#6A77D7">2) Table Creation </Highlight> 챕터에서 생성된 테이블의 column 이름과 일치하도록 `rename` 함수를 이용해 수정합니다.
...
2. `2) Table Creation` 챕터에서 작성한 `iris_data` 테이블에 있는 데이터 전체를 확인하기 위한 query 를 실행합니다.
    ...
    `2) Table Creation` 챕터과는 다르게 데이터가 한 줄 추가된 것을 확인할 수 있습니다.
