...
1. Docker 를 이용해 postgres 서버를 생성합니다.
...
`-d` 옵션을 사용하면 컨테이너가 detached 모드로 실행하게 되며, `-d` 옵션 없이 실행했다면 해당 터미널에서 `Ctrl + C` 를 눌러서 빠져나오는 순간 해당 컨테이너는 종료됩니다.
...
`docker ps` 명령어를 통해 postgres 서버가 잘 생성되었는지 확인합니다.
...
Postgres 서버를 확인할 때 사용하는 CLI 툴을 설치합니다.  
