...
`@app.get()`, `@app.post()` 등 다양한 path operations 에 `response_model` 을 이용하여 Response Body 에 사용될 데이터 모델을 지정해줄 수 있습니다.
...
`USER_DB[user.name] = user.nickname` 와 같이 Pydantic 의 모델에 선언된 변수를 사용하여 DB 에 사용자 정보를 저장해 줍니다.
...
위와 같이 Pydantic Model 을 통해 Response Model 로 지정하여 Create API 를 수정하였습니다.
이처럼 Pydantic Model 을 통해 Response Model 을 사용하면 입력 받는 파라미터와 생성 후 반환하는 파라미터를 다르게 지정해 줄 수 있습니다.
이러한 기능은 비밀번호처럼 사용자가 필수적으로 입력해야 하지만 반환 값에는 나타나면 안되는 파라미터를 지정할 때 유용하게 사용될 수 있습니다.
