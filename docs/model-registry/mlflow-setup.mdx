...
- <var>image</var> : 
  ...
  - MinIO 서버에서 사용할 이미지는 <code>minio/minio</code> 입니다.
- <var>ports</var> :
...
- <var>command</var> : MinIO 서버를 실행시키는 명령어를 추가합니다. <code>--console-address</code> 를 통해 컨테이너의 9001 포트로 MinIO 에 접근할 수 있도록 주소를 열어줍니다.
- <var>healthcheck</var> : MinIO 서버가 잘 띄워졌는지 상태를 확인하기 위해 상태를 체크하는 구문을 추가합니다.
