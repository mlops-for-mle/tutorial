...
2. Docker Compose 파일에 학습된 모델을 저장할 물리적인 저장 공간인 MinIO 서버 스펙을 정의 합니다.
...
3. Docker Compose 파일에 모델과 모델의 결과들을 관리할 MLFlow 서버를 정의 합니다.
    - 앞서 띄워둔 `PostgreSQL DB` 와 `MinIO` 두 가지 서버를 연결 합니다.  ** 연결한다는 표현이 조금 모호한 것 같습니다. 조금 더 구체적으로 알려주면 좋을 것 같은데요. 다른 표현이 있을까요?**
    - Dockerfile
...
4. 정의된 스펙에 따라 Docker Compose 를 활용해 서비스들을 띄웁니다.
...
Backend Store 란 수치 데이터와 MLflow 서버의 정보들을 체계적으로 관리하기위한 DB 입니다. Backend Store 에는 모델의 학습 결과인 accuracy, f1-score, 모델이 학습되면서 생기는 loss, 모델 자체의 정보인 hyperparmameters 등의 수치 데이터와 run_id, run_name, experiment_name 등의 MLflow 의 메타 데이터가 저장됩니다.

이번 파트에서는 Backend Store 를 위해 `01. Database` 파트에서 사용되었던 PostgreSQL DB 를 새롭게 생성하겠습니다.

### 1.2 PostgreSQL DB Server

PostgreSQL DB 서버의 스펙을 Docker Compose 파일에 서비스 이름, 사용자, 비밀번호, DB 이름을 환경변수를 설정합니다.
...
- `image` : DB 서버에서 사용할 이미지는 `postgres:14.0` 입니다.
- `environment` :
...
- `healthcheck` : DB 서버가 잘 띄워졌는지 상태를 확인하기 위해 상태를 체크합니다.
...
Artifact Store 란 MLflow 에서 학습된 모델을 저장하는 model registry 로 이용하기 위한 스토리지 (storage) 서버입니다. Artifact Store 를 이용하면 기본적인 파일 시스템 보다 체계적으로 관리 할 수 있으며 외부에 있는 스토리지 서버도 사용 할 수 있다는 장점이 있습니다. 
...
MinIO의 스펙을 Docker Compose 파일에 서비스 이름, 사용자, 비밀번호를 환경변수로 정의하고 호스트와 연결되는 포트 또한 정의합니다.
...
- <code>image</code> : MinIO 서버에서 사용할 이미지는 <code>minio/minio</code> 입니다.
- <code>ports</code> : 호스트와 컨테이너의 포트를 설정합니다.
...
- <code>command</code> : MinIO 서버를 실행시키는 명령어를 추가합니다. `--console-address` 를 통해 컨테이너의 9001 포트로 MinIO 에 접근할 수 있도록 주소를 열어줍니다.
- <code>healthcheck</code> : MinIO 서버가 잘 띄워졌는지 상태를 확인하기 위해 상태를 체크하는 구문을 추가합니다.

</CodeDescription>

## 3. MLflow Server

앞서 만든 Backend Store 와 Artifact Store 에 연결된 MLflow Server 를 생성합니다.
...
작성된 Dockerfile 을 build 하도록 Docker Compose 파일의 `services` 탭 밑에 정의합니다. 
  1. MinIO 에 연결하기 위한 계정 정보를 환경변수로 설정합니다.
...
- `build` : 
  - 앞서 작성한 Dockerfile 을 사용하여 이미지를 build 하여 사용합니다.
- `depends_on` : MLflow 서버가 띄워지기 전에, `PostgreSQL DB` 와 `MinIO` 서버를 먼저 띄우도록 합니다.
  - `condition` : `PostgreSQL DB` 와 `MinIO` 서비스의 동작 상태가 체크되면 MLflow 서버를 띄웁니다.
- `ports` : 
...
- `command` : MinIO 초기 버켓을 생성 하고, MLflow 서버를 실행합니다.
  ...
  - `--backend-store-uri` 에 명시된 정보를 통해 PostgreSQL DB 와 연결합니다.
...
완성된 Dockerfile 과 Docker Compose 파일은 아래와 같습니다.
...
Dockerfile 과 Docker Compose 두 파일이 있는 경로에서 아래 명령어를 활용해 정의된 서비스를 백그라운드 프로세스로 띄웁니다.
