...
Backend Store 란 수치 데이터와 MLflow 서버의 정보들을 체계적으로 관리하기위한 DB 입니다. 
...
PostgreSQL DB 서버의 스펙을 Compose 파일에 서비스 이름, 사용자, 비밀번호, DB 이름을 환경변수를 설정합니다.
...
Artifact Store 란 MLflow 에서 학습된 모델을 저장하는 model registry 로 이용하기 위한 스토리지 (storage) 서버입니다. 
...
MinIO의 스펙을 Compose 파일에 서비스 이름, 사용자, 비밀번호를 환경변수로 정의하고 호스트와 연결되는 포트 또한 정의합니다.
