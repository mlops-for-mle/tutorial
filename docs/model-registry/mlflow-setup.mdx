...
3. Docker Compose 파일에 모델과 모델의 결과들을 관리할 MLFlow 서버를 정의 합니다.
    ...
    - Docker Compose
      ...
      - Port forwarding : <code>5001:5000</code>
        ...
        - 하지만 **MacOS** 의 경우 5000번 포트로 "AirPlay" 기능을 사용하기 때문에 이런 경우 보편적인 적용을 위해 5001번 포트를 사용합니다.
...
Backend Store 에는 모델의 학습 결과인 accuracy, f1-score, 모델이 학습되면서 생기는 loss, 모델 자체의 정보인 hyperparmameters 등의 수치 데이터와 run_id, run_name, experiment_name 등의 MLflow 의 메타데이터가 저장됩니다.

이번 파트에서는 Backend Store 로 사용하기 위해 `01. Database` 파트에서 사용되었던 PostgreSQL DB 를 새롭게 생성하겠습니다.

### 1.2 PostgreSQL DB Server

PostgreSQL DB 서버의 스펙을 Compose 파일에 서비스 이름, 사용자, 비밀번호, DB 이름을 환경변수로 설정합니다.
