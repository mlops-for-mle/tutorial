...
앞서 받은 두 가지 변수를 `mlflow.sklearn.load_model` 의 인자에 설정하여 모델을 불러옵니다.
...
#### 1.2.2 `sklearn` 모델 불러오기

**이 부분 sklearn 으로 변경되어서 글을 바꿔야할 것 같습니다.**
...
마찬가지로 앞서 받은 두 가지 변수를 `mlflow.sklearn.load_model` 의 인자에 매핑하여 모델을 로드합니다.
