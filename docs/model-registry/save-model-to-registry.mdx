...
- `experiment` : MLflow 에서 정보를 관리하기 위해 나누는 일종의 directory 입니다. `BERT`, `ResNet` 과 같이 특정 이름으 통해 생성 할 수 있으며, 생성하지 않고 MLflow 에 정보를 저장하는 경우 `Default` 라는 이름의 `experiment` 에 저장됩니다.
