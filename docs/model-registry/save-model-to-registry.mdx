...
1. <Part>02. Model Development</Part> 파트에서 사용한 코드를 이용해 모델을 학습합니다.
2. 학습이 끝난 모델을 mlflow 의 built-in method 를 사용해 mlflow server 에 저장합니다.
    - Python의 `mlflow` 패키지를 이용합니다.
        - `pip install mlflow`
    - `mlflow` 를 활용해 모델을 <Chapter>1) MLflow Setup</Chapter> 챕터에서 띄운 mlflow server 에 저장합니다.
...
이 정보는 <Chapter>1) MLflow Setup</Chapter> 챕터의 `docker-compose.yaml` 에서 설정한 `mlflow-server` , `mlflow-artifact-store` 의 정보와 같습니다. 접근에 사용할 ID, PW 는 사전에 정의된 시스템 환경 변수에 매핑하여 Artifact-store 에 접근 할 수 있습니다. 같은 방식으로 서비스가 띄워져있는 MLflow 서버와 S3(MinIO) 의 URI도 함께 매핑해 줍니다.
...
- `MLFLOW_S3_ENDPOINT_URL` : 모델을 저장할 storage 의 주소입니다. <Chapter>1) MLflow Setup</Chapter>  챕터에서 띄운 `MinIO` 서버 주소와 같으며 `http://localhost:9000` 입니다.
- `MLFLOW_TRACKING_URI` : 정보를 저장하기 위해 연결 할 MLflow 서버의 주소 입니다. <Chapter>1) MLflow Setup</Chapter>  챕터의 MLflow 서버 주소와 같으며 `http://localhost:5001` 입니다.
- `AWS_ACCESS_KEY_ID` : `MinIO` 에 접근하기 위한 ID 입니다. <Chapter>1) MLflow Setup</Chapter>  챕터에서 설정한 `MINIO_ROOT_USER` 인 `minio` 를 사용합니다.
- `AWS_SECRET_ACCESS_KEY` : `MinIO` 에 접근하기 위한 PW 입니다. <Chapter>1) MLflow Setup</Chapter>  챕터에서 설정한 `MINIO_ROOT_PASSWORD` 인 `miniostorage` 를 사용합니다.
...
이어서 <Part>02. Model Development</Part> 파트의 **모델**과 **모델의 결과 metric** 인 정확도를 저장해 보겠습니다.  
