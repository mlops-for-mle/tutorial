...
1. Kafka Topic 에서 데이터를 가져올 수 있는 코드를 작성합니다.
2. 가져온 데이터를 API Serving 에 요청을 보내 예측값을 받습니다.
3. 받아온 예측값을 target postgres 서버에 삽입합니다.
...
&nbsp; <Part>07. Kafka</Part> 에서는 최종적으로 source connector 와 sink connector 를 생성하여 source DB 에서 target DB 로 데이터를 전달하는 과정을 살펴봤습니다. <Part>08. Stream</Part> 에서는 다시 model deployment 관점으로 돌아와서 kafka 를 어떻게 쓸지 살펴보겠습니다.
이전과 동일하게 zookeeper, broker, connect 를 쓰는 것은 동일합니다. 그리고 source connector 를 사용하여 데이터를 publish 하는 과정도 동일합니다. 달라지는 점은 sink connector 를 대신해서 직접 kafka 의 python SDK 를 이용하여 consumer 를 구현하는 것입니다.
<p></p>
&nbsp; 왜 직접 구현해서 사용해야할까요? <Part>07. Kafka</Part> 에서 사용한 sink connector 를 살펴 보면, JSON 파일틀 통해 사전에 정의 된 양식에 맞추어 생성한 후 사용합니다.
...
[그림 8-2]은 docker-compose 파일에 작성될 서비스들을 포함한 전체 다이어그램입니다.
...
- `psycopg2-binary` : 앞선 파트들에서 사용한 것처럼 python 에서 DB 에 접근할 때 사용하는 패키지입니다. API 에서 받은 response 결과를 target DB 에 추가할 때 사용합니다.
