...
3. <Part>07. Kafka</Part> 에서 띄웠던 Target DB 를 확인하고 Grafana 에서 시각화합니다.
...
- <var>environment</var> :
    ...
    - <var>GF_DASHBOARDS_MIN_REFRESH_INTERVAL</var> : 
        ...
        - 설정하지 않는 경우 기본값은 <code>5s</code> 의 값을 가지며, 5초 이하의 주기로 새로고침 되도록 설정할 수는 없습니다.  **5초 이하로는 안되는데 500ms를 해도 되나요..? 말이 좀 이상한 것 같아요(?)**
